...
import { DisclosureProvider, Transition } from "@atomic_ui_react/mod.ts";
...
Transition can be used to achieve beautiful opening and closing movements.

export const DisclosureProviderWithTransitionExample = () => {
  return (
    <div className="bg-white p-2 rounded-xl space-y-2">
      {contents.map(({ title, content }) => (
        <DisclosureProvider key={title}>
...
<Playground preview={<DisclosureProviderWithTransitionExample />}>

```tsx
import { DisclosureProvider, Transition } from "https://deno.land/x/atomic_ui_react@$VERSION/mod.ts"
